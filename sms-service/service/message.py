...
def send_message(user_id, response_id, title, to_number):
    try:
        body = "Hi ! Your account with user_id {} has received form response id: {} for form titled : {}".format(
            user_id, response_id, title
        )
        message = client.messages.create(
            body=body, from_=from_number, to=remove(to_number)
        )
